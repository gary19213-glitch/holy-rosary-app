...
export default function Home() {
  const [screen, setScreen] = useState("home"); // home, rosary, chaplet, stations, library, settings, journal, stats
  ...
  // PRAYER TIME TRACKER (Feature 63 - FIXED to use Seconds globally!)
  ...
  const [totalChaplets, setTotalChaplets] = useState(0); // FEATURE 62
  ...
  // FEATURE 87: RESET STATS
  const resetStats = () => {
    if (confirm("Are you sure you want to delete all prayer history?")) {
  ...
  const finishAndSave = () => {
    ...
    setTotalPrayerSeconds(newSecs); localStorage.setItem("totalPrayerSeconds", newSecs.toString());
  ...
  return (
    <div style={{ padding: "20px", backgroundColor: screen === "chaplet" ? "#3a0e1b" : actualColor, color: "white", minHeight: "100vh", transition: "background-color 1s ease" }}>
        ...
        <>
          ...
          {todaysSaint && (
            <div style={{ backgroundColor: "rgba(0,0,0,0.2)", padding: "15px", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.1)", marginBottom: "20px", display: "flex", alignItems: "center", gap: "15px" }}>
              ...
              <div><p style={{ color: "#d4af37", fontSize: "12px", fontWeight: "bold", textTransform: "uppercase", margin:0 }}>{todaysSaint.type}</p><h3 style={{ fontSize: "16px", margin: "3px 0" }}>{todaysSaint.name}</h3><p style={{ fontSize: "13px", color: "#ccc", margin:0 }}>{todaysSaint.bio}</p></div>
            </div>
          )}
      ...
      {/* --- STATS DASHBOARD (Features 62, 64, 66) --- */}
      {screen === "stats" && (
        <>
      ...
      {screen === "settings" && (
        <><header style={{ display: "flex", alignItems: "center", marginBottom: "30px" }}><button onClick={() => setScreen("home")} style={{ fontSize: "16px", background: "none", color: "#a0a0a0", border: "none", marginRight: "20px" }}>← Back</button><h1 style={{ fontSize: "24px" }}>Settings</h1></header>
          ...
          <button onClick={resetStats} style={{ width: "100%", padding: "15px", backgroundColor: "#3a0e1b", color: "white", border: "1px solid #ff4444", borderRadius: "12px", fontWeight: "bold", marginTop: "20px" }}>Reset All Data & Stats</button>
        </>
      )}
      ...
      {screen === "journal" && (
        <div style={{ paddingBottom: "100px", textAlign: "center", marginTop: "40px" }}>
          <h1 style={{ fontSize: "60px", margin: 0 }}>🕊️</h1><h2 style={{ fontSize: "28px", color: "#d4af37", marginBottom: "10px" }}>Prayer Complete!</h2><p style={{ color: "#ccc", fontSize: "16px", marginBottom: "30px" }}>Time prayed: {formatTime(sessionSeconds)}</p>
          <div style={{ textAlign: "left", backgroundColor: "rgba(0,0,0,0.4)", padding: "24px", borderRadius: "16px", border: "1px solid #333", marginBottom: "30px" }}><h3 style={{ fontSize: "18px", color: "#d4af37", marginBottom: "10px" }}>Spiritual Journal</h3><p style={{ fontSize: "14px", color: "#a0a0a0", marginBottom: "15px" }}>Write down any graces, thoughts, or intentions from this prayer session.</p><textarea value={journalEntry} onChange={(e) => setJournalEntry(e.target.value)} placeholder="Dear Lord..." style={{ width: "100%", height: "150px", padding: "12px", borderRadius: "8px", backgroundColor: "#1a1a2e", color: "white", border: "1px solid #444", fontSize: "16px", fontFamily: "inherit", resize:"none" }} /></div>
