...
function RosaryBeads({ currentBead }: { currentBead: number }) {
  ...
  const circleBeads = Array.from({ length: 55 }).map((_, i) => {
    ...
    return {
      ...
      isLarge,
      isActive: currentBead === (i < 11 ? i + 1 : 0) // Simplified active logic for visual effect
    };
  });
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '20px 0' }}>
      <svg width="300" height="420" viewBox="0 0 300 420">
        ...
        {circleBeads.map((b) => (
          <circle 
            key={`c_${b.id}`} cx={b.x} cy={b.y} r={b.isLarge ? 7 : 4} 
            fill={b.isLarge ? "#888" : "#444"}
            stroke="none" strokeWidth="1.5"
          />
        ))}

        <circle cx="150" cy="280" r={7} fill="#888" />
        <circle cx="150" cy="300" r={4} fill="#444" />
        <circle cx="150" cy="315" r={4} fill="#444" />
        <circle cx="150" cy="330" r={4} fill="#444" />
        <circle cx="150" cy="350" r={7} fill="#888" />
        <text x="150" y="405" fontSize="40" textAnchor="middle" fill="#d4af37">✝</text>

        {/* The Golden Highlight Bead that moves! */}
        <circle 
          cx={150} cy={160} r={10} fill="#d4af37" 
          style={{ filter: "drop-shadow(0px 0px 10px #d4af37)" }} 
        />
        <text x="150" y="195" fontSize="12" textAnchor="middle" fill="#d4af37">Active Bead</text>
      </svg>
    </div>
  );
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (appTheme === "auto") { 
      setActualColor(getLiturgicalInfo().color); 
    }
  ...
  const finishAndSave = () => {
    if (screen === "rosary") { 
      const n = totalRosaries + 1; 
      setTotalRosaries(n); 
      localStorage.setItem("totalRosaries", n.toString()); 
    }
    if (screen === "chaplet") { 
      const n = totalChaplets + 1; 
      setTotalChaplets(n); 
      localStorage.setItem("totalChaplets", n.toString()); 
    }
    ...
    const newSecs = totalPrayerSeconds + sessionSeconds; 
    setTotalPrayerSeconds(newSecs); 
    localStorage.setItem("totalPrayerSeconds", newSecs.toString());
    ...
    if (!monthlyDays.includes(todayStr)) {
      ...
      setMonthlyDays(newMonthly); 
      localStorage.setItem("monthlyDays", JSON.stringify(newMonthly));
    }
  ...
  const nextPrayer = () => {
    ...
    if (screen === "stations") { 
      if (currentStation < stationsOfCross.length - 1) {
    ...
    if (stage === "intro") { 
      ...
      if (pendantBead < 4) {
        setPendantBead(pendantBead + 1); 
      } else { 
        setStage("decades"); setCurrentDecade(0); setCurrentBead(0); 
      } 
    } 
      ...
      if (currentBead < 11) {
        ...
        if (currentDecade < 4) { 
          setCurrentDecade(currentDecade + 1); setCurrentBead(0); 
        } else {
          setStage("outro"); 
        }
      } 
        ...
        <div style={{ maxWidth: "600px", margin: "0 auto", paddingBottom: "120px" }}>
          ...
          {stage === "intro" && ( 
            <div style={{ textAlign: "center", marginTop: "40px", animation: "fadeIn 1s ease" }}>
              <h2 style={{ fontSize: "28px", color: goldColor, marginBottom: "30px", fontWeight: "normal" }}>Opening Prayers</h2>
              <div style={{ textAlign: "left", backgroundColor: cardBg, padding: "30px", borderRadius: "16px", border: `1px solid ${borderColor}`, boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
                {renderPrayerText(screen === "rosary" ? prayers.signOfCross : chapletPrayers.opening)}
              </div>
            </div> 
          )}
          ...
          {stage === "decades" && (
            <div style={{ animation: "fadeIn 1s ease", marginTop: "30px" }}>
              ...
              <div style={{ textAlign: "left", backgroundColor: cardBg, padding: "30px", borderRadius: "16px", border: `1px solid ${borderColor}`, position: "relative", marginTop: displayMode === "text" ? "20px" : "0", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
                ...
                <h3 style={{ color: goldColor, marginBottom: "25px", textAlign: "center", fontSize:"24px", fontWeight: "normal" }}>
                  {screen === "rosary" ? (currentBead === 0 ? "The Our Father" : currentBead === 11 ? "The Glory Be" : `Hail Mary (${currentBead}/10)`) : (currentBead === 0 || currentBead === 11 ? "Large Bead" : `Small Bead (${currentBead}/10)`)}
                </h3>
                ...
                <div style={{ fontSize: "18px" }}>
                  ...
                  {screen === "rosary" && currentBead === 11 && customDecadePrayer && (
          ...
          {stage === "outro" && ( 
            <div style={{ textAlign: "center", marginTop: "40px", animation: "fadeIn 1s ease" }}>
              ...
              <div style={{ textAlign: "left", backgroundColor: cardBg, padding: "30px", borderRadius: "16px", border: `1px solid ${borderColor}`, boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
                ...
                {screen === "rosary" && customClosingPrayer && (
                  <div style={{ marginTop: "30px", padding: "20px", backgroundColor: isLight ? "#f9f9f9" : "rgba(0,0,0,0.2)", borderRadius: "8px", borderLeft: `3px solid ${goldColor}` }}>
          ...
          <div style={{ position: "fixed", bottom: "30px", left: "50%", transform: "translateX(-50%)", width: "calc(100% - 40px)", maxWidth: "560px", zIndex: 50 }}>
            <button onClick={nextPrayer} style={{ backgroundColor: goldColor, color: isLight ? "#fff" : "#1a1a2e", padding: "18px", width: "100%", borderRadius: "30px", fontSize: "16px", fontWeight: "bold", border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: "1px", boxShadow: "0 10px 20px rgba(0,0,0,0.3)" }}>
              {stage === "outro" ? "Finish ✓" : "Next Prayer ➔"}
            </button>
          </div>
        </div>
      );})()}

      {/* REMAINDER COMPRESSED SCREENS: Stats, Journal, Stations, Library, Lists */}
